...
function Process(_htm){
  ...
  try{
    ...
    $(".J_result").html("<PRE class='CodeContainer'>"+html+"</PRE>");
  }catch(e){
    alert("JSON数据格式不正确:\n"+e.message);
    return  $(".J_result").html("");
  }
}
  ...
  $id("InvisibleLink").submit();
    ...
    return {
        init: function (_htm) {
            _htm = _htm;
            Process(_htm);
            window.ExpImgClicked = ExpImgClicked;
            window.ExpandAllClicked = ExpandAllClicked;
            window.CollapseAllClicked = CollapseAllClicked;
